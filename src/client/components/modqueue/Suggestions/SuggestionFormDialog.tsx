import { useEffect, useState, useMemo, FormEvent, useRef, DragEvent } from 'react';
...
interface SuggestionFormDialogProps {
  ...
  initialValues?: SuggestionFormValues | undefined;
  submitting: boolean;
  onClose: () => void;
...
export const SuggestionFormDialog = ({
  isOpen,
  isEditing,
  categories,
  initialValues,
  ...
  }, [categories, initialValues, isOpen, submitting]);
  ...
  const notesCount = `${notes.length}/${NOTES_LIMIT}`;
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-white/20 bg-[#0b0f11] px-4 py-6 shadow-[0_20px_60px_rgba(0,0,0,0.45)] mt-4">
        ...
        <div className="space-y-1.5">
          <h3 className="text-xl font-semibold text-white">
            {titleOverride ?? (isEditing ? 'Edit Suggestion' : 'Suggest Item')}
          </h3>
          <p className="text-sm text-slate-300">
            {subtitleOverride ?? 'Add the item details and upload the image directly here.'}
          </p>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                placeholder="Add a additional note to the admin"
              />
            </div>
          </div>
